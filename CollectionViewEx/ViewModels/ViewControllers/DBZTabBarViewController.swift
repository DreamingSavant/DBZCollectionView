...
class DBZTabBarViewController: UITabBarController {
    ...
    let thirdTab: DBZCharactersExpandableCollectionViewController = {
        let expandableVC = DBZCharactersExpandableCollectionViewController()
        let cardImage = UIImage(systemName: "rectangle.stack.person.crop.fill")?.withTintColor(.black)
        let selectedCardImage = UIImage(systemName: "rectangle.stack.person.crop.fill")?.withTintColor(.blue)
        let tabBarItem = UITabBarItem(title: "Explore", image: cardImage, selectedImage: selectedCardImage)
        expandableVC.tabBarItem = tabBarItem
        return expandableVC
    }()
